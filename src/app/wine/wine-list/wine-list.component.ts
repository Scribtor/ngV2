...
export class WineListComponent implements OnInit {
  ...
  public izmeniStranuPaginacije(javljenaStranicaPaginacije:number)
  {
    ...
    console.log(javljenaStranicaPaginacije);    //2
  ...
  odrediKrajnjiIndex(pIndexFParam:number,paramFBrojelemenataPoStranici:number,paramFPoslatLimit:number):number
  {
    let tmp:number = pIndexFParam+paramFBrojelemenataPoStranici;
    if (
          (pIndexFParam+paramFBrojelemenataPoStranici)>paramFPoslatLimit
       ) {
      tmp=pIndexFParam + (paramFPoslatLimit-pIndexFParam);
      return tmp;
    }
    return tmp;
